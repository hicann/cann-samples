...
// zero 和 one 分别写 BF16 寄存器的低半段和高半段, 后续通过 Or 合并.
...
// 输入 Sᵀ[Bc, halfBr] 为 DN 布局, 每行占 1 个 64-lane FP32 寄存器.
// 沿 Bc 维逐 lane 计算 max/sum, 每个 lane 对应本 AIV 的 1 个 Q 行.
template <bool IS_FIRST_ITER>
__simd_vf__ inline void
OnlineColwiseSoftmaxVF(__ubuf__ float *sUBAddr, __ubuf__ float *mUBAddr,
                       __ubuf__ float *lUBAddr, __ubuf__ float *alphaUBAddr,
                       const uint16_t halfBr, const uint16_t bc,
                       const float scale) {
    ...
    if constexpr (!IS_FIRST_ITER) {
        ...
        Reg::StoreAlign<float, Reg::StoreDist::DIST_NORM_B32>(alphaUBAddr,
                                                              alpha, all);
    }
    ...
    for (uint16_t r = 0; r < bc; ++r) {
        ...
        Reg::StoreAlign<float, Reg::StoreDist::DIST_NORM_B32>(
            sUBAddr + r * halfBr, expReg, all);
    }
    if constexpr (IS_FIRST_ITER) {
        Reg::Duplicate(alpha, 1.0f, all);
        Reg::StoreAlign<float, Reg::StoreDist::DIST_NORM_B32>(alphaUBAddr,
                                                              alpha, all);
        Reg::StoreAlign<float, Reg::StoreDist::DIST_NORM_B32>(lUBAddr, sumReg,
                                                              all);
    } else {
        Reg::LoadAlign(lOld, lUBAddr);
        Reg::Mul(lOld, lOld, alpha, all);
        Reg::Add(lOld, lOld, sumReg, all);
        Reg::StoreAlign<float, Reg::StoreDist::DIST_NORM_B32>(lUBAddr, lOld,
                                                              all);
    }
}

// 将 DN[Bc, col] 的 FP32 exp(Pᵀ) 转为带 32B 组间 padding 的 BF16 NZ.
// Br % 128 == 0 时 col=Br/2 为 64 的倍数. 每次处理 1 个 64-lane FP32
// 寄存器, Cast 后由 Pack 压紧为连续 BF16 元素.
__simd_vf__ inline void FusedDNToNZCastVF(__ubuf__ bfloat16_t *pDstAddr,
                                          __ubuf__ float *pSrcAddr,
                                          const uint16_t col,
                                          const uint16_t row) {
    using namespace AscendC;
    ...
    Reg::MaskReg b16HalfMask =
        Reg::CreateMask<bfloat16_t, Reg::MaskPattern::H>();
    // 每个 q1 组含 Bc 个有效 DataBlock, 末尾保留 1 个 32B padding.
    const uint32_t groupStrideBlocks = row + 1;

    __ubuf__ bfloat16_t *dstGroupAddr;
    for (uint16_t qBase = 0; qBase < col; qBase += VL_B32) {
        dstGroupAddr = pDstAddr + (qBase / B16_PER_DATABLOCK) *
                                      groupStrideBlocks * B16_PER_DATABLOCK;
        for (uint16_t k = 0; k < row; ++k) {
            ...
            Reg::Pack(reinterpret_cast<Reg::RegTensor<uint16_t> &>(packed),
                      reinterpret_cast<Reg::RegTensor<uint32_t> &>(bf16));
            // 每次写当前 K 的 4 个 q1 DataBlock. POST_MODE_UPDATE 将
            // 下一次写地址前移到 K+1.
            Reg::StoreAlign<bfloat16_t, Reg::DataCopyMode::DATA_BLOCK_COPY,
                            Reg::PostLiteral::POST_MODE_UPDATE>(
                dstGroupAddr, packed, groupStrideBlocks, 1, b16HalfMask);
        }
    }
}

// 寄存器内计算 O_acc = alpha * O_acc + ΔO. 首轮 O_acc 为 0,
// 因此无需区分 j==0. alpha 通过 BRC 按行广播.
__simd_vf__ inline void OnlineUpdateVF(__ubuf__ float *oAccUBAddr,
                                       __ubuf__ float *oDeltaUBAddr,
                                       __ubuf__ float *alphaUBAddr,
                                       const uint16_t halfBr,
                                       const uint16_t d) {
    using namespace AscendC;
    ...
    for (uint16_t i = 0; i < halfBr; ++i) {
        Reg::LoadAlign<float, Reg::LoadDist::DIST_BRC_B32>(alpha,
                                                           alphaUBAddr + i);
        for (uint16_t j = 0; j < dLoops; ++j) {
            ...
            Reg::StoreAlign<float, Reg::StoreDist::DIST_NORM_B32>(
                oAccUBAddr + i * d + j * floatRepSize, add, pregAll);
        }
    }
}

// 计算 O_acc / l 并转为 BF16 ND 布局, 供 DataCopy 写回 GM.
// l 通过 BRC 按行广播, DINTLV_B32 每次读取 2 个 FP32 寄存器.
__simd_vf__ inline void FusedDivCastVF(__ubuf__ bfloat16_t *dstUBAddr,
                                       __ubuf__ float *srcUBAddr,
                                       __ubuf__ float *lUBAddr,
                                       const uint16_t halfBr,
                                       const uint16_t d) {
    using namespace AscendC;
    ...
    for (uint16_t i = 0; i < halfBr; ++i) {
        ...
        for (uint16_t c = 0; c < dLoops; ++c) {
            Reg::LoadAlign<float, Reg::LoadDist::DIST_DINTLV_B32>(
                srcEven, srcOdd, srcUBAddr + i * d + c * dintlvLen);
            Reg::Div(divEven, srcEven, lReg, pregAll);
            Reg::Div(divOdd, srcOdd, lReg, pregAll);
            Reg::Cast<bfloat16_t, float, castTraitZero>(castEven, divEven,
                                                        pregAllB16);
            Reg::Cast<bfloat16_t, float, castTraitOne>(castOdd, divOdd,
                                                       pregAllB16);
            Reg::Or(reinterpret_cast<Reg::RegTensor<uint16_t> &>(packed),
                    reinterpret_cast<Reg::RegTensor<uint16_t> &>(castEven),
                    reinterpret_cast<Reg::RegTensor<uint16_t> &>(castOdd),
                    pregAllB16);
...
// V1 计算: online softmax -> P 转 NZ.
__aicore__ inline void
VectorStage1(AscendC::LocalTensor<float> &sUBLocal,
             AscendC::LocalTensor<float> &mUBLocal,
             AscendC::LocalTensor<float> &lUBLocal,
             AscendC::LocalTensor<float> &alphaUBLocal,
             AscendC::LocalTensor<bfloat16_t> &pWorkUBLocal,
             const FlashAttnLiteTilingData &data, uint32_t j) {
    using namespace AscendC;

    if ASCEND_IS_AIV {
        __ubuf__ float *sUBAddr =
            reinterpret_cast<__ubuf__ float *>(sUBLocal.GetPhyAddr());
        __ubuf__ float *mUBAddr =
            reinterpret_cast<__ubuf__ float *>(mUBLocal.GetPhyAddr());
        __ubuf__ float *lUBAddr =
            reinterpret_cast<__ubuf__ float *>(lUBLocal.GetPhyAddr());
        __ubuf__ float *alphaUBAddr =
            reinterpret_cast<__ubuf__ float *>(alphaUBLocal.GetPhyAddr());
        if (j == 0) {
            asc_vf_call<OnlineColwiseSoftmaxVF<true>>(
                sUBAddr, mUBAddr, lUBAddr, alphaUBAddr,
                static_cast<uint16_t>(data.br / 2),
        ...
        // 每个 NZ q1 组后保留 1 个 32B padding, 由 UB -> L1 DataCopy 跳过.
        asc_vf_call<FusedDNToNZCastVF>(
            reinterpret_cast<__ubuf__ bfloat16_t *>(pWorkUBLocal.GetPhyAddr()),
            reinterpret_cast<__ubuf__ float *>(sUBLocal.GetPhyAddr()),
            static_cast<uint16_t>(data.br / 2), static_cast<uint16_t>(data.bc));
    }
}

__aicore__ inline void
CopyPWorkToL1(const AscendC::LocalTensor<bfloat16_t> &dstL1Local,
              const AscendC::LocalTensor<bfloat16_t> &srcUBLocal, uint32_t hBr,
              uint32_t bc) {
    using namespace AscendC;
    const uint16_t blockCount = static_cast<uint16_t>(hBr / B16_PER_DATABLOCK);
    const uint16_t blockLen = static_cast<uint16_t>(bc);
    // 每个 q1 组搬运 Bc 个 DataBlock, 跳过源端末尾的 padding.
    DataCopyParams copyParams(blockCount, blockLen, 1, 0);
    DataCopy(dstL1Local, srcUBLocal, copyParams);
}

// V2 计算: 更新 O_acc = alpha * O_acc + ΔO.
__aicore__ inline void VectorStage2(AscendC::LocalTensor<float> &oAccUBLocal,
    ...
    if ASCEND_IS_AIV {
        ...
        asc_vf_call<OnlineUpdateVF>(
            ...
            reinterpret_cast<__ubuf__ float *>(alphaUBLocal.GetPhyAddr()),
            static_cast<uint16_t>(halfBr), static_cast<uint16_t>(HEAD_DIM));
    }
}

__aicore__ inline void KernelProcessForAIV(__gm__ bfloat16_t *outGMAddr,
    ...
    if ASCEND_IS_AIV {
        ...
        // 两路 AIV 分别写共享 P L1 的一半, 地址由 host tiling 规划.
        LocalTensor<bfloat16_t> pL1Local(TPosition::A1, aic.pL1Addr,
                                         aic.pL1Elems);
        // UB 按 S/ΔO/O_acc/PWork/m/l/alpha 排布.
        ...
        LocalTensor<float> oDeltaUBLocal(TPosition::VECCALC, aiv.oDeltaUBAddr,
                                         aiv.oDeltaUBElems);
        LocalTensor<float> oAccUBLocal(TPosition::VECCALC, aiv.oAccUBAddr,
                                       aiv.oAccUBElems);
        // PWork 在 j 循环中保存带 padding 的 NZ P, 循环后复用为 ND O.
        LocalTensor<bfloat16_t> pWorkUBLocal(TPosition::VECCALC,
                                             aiv.pWorkUBAddr, aiv.pWorkUBElems);
        LocalTensor<float> mUBLocal(TPosition::VECCALC, aiv.mUBAddr,
                                    aiv.rowStatsUBElems);
        LocalTensor<float> lUBLocal(TPosition::VECCALC, aiv.lUBAddr,
                                    aiv.rowStatsUBElems);
        LocalTensor<float> alphaUBLocal(TPosition::VECCALC, aiv.alphaUBAddr,
                                        aiv.rowStatsUBElems);

        for (uint32_t taskId = aicIdx; taskId < data.numTasks;
             taskId += data.useAicNum) {
            const uint32_t batchIdx = taskId / data.tr;
            const uint32_t tileIdx = taskId % data.tr;
            const uint64_t outGMOffset =
            ...
            for (uint32_t j = 0; j < data.tc; ++j) {
                ...
                // 两路 AIV 使用本地 ID, 硬件分别映射到 AIC 的 ID 和 ID+16.
                CrossCoreWaitFlag<PAIR_CROSS_MODE, PIPE_V>(FLAG_S_READY);
#else
                CrossCoreWaitFlag<GROUP_CROSS_MODE, PIPE_V>(FLAG_S_READY);
#endif
                VectorStage1(sUBLocal, mUBLocal, lUBLocal, alphaUBLocal,
                             pWorkUBLocal, data, j);
                SetWaitFlag<HardEvent::V_MTE3>(STATIC_EVENT_ID0);
                auto pSliceL1Local =
                    pL1Local[static_cast<uint64_t>(subAivIdx) * pHalfElements];
                CopyPWorkToL1(pSliceL1Local, pWorkUBLocal, halfBr, bc);
                // Set 绑定生产 P 的 MTE3, 确保 UB -> L1 完成后再上报.
#ifdef SIM_COMPATIBLE
                // CANNsim 兼容路径使用 mode4 分别上报两路 AIV.
                CrossCoreSetFlag<PAIR_CROSS_MODE, PIPE_MTE3>(FLAG_P_READY);
#else
                // mode2 在两路 AIV 均 Set 后聚合为 1 次通知.
                CrossCoreSetFlag<GROUP_CROSS_MODE, PIPE_MTE3>(FLAG_P_READY);
#endif

#ifdef SIM_COMPATIBLE
                CrossCoreWaitFlag<PAIR_CROSS_MODE, PIPE_V>(FLAG_O_READY);
#else
                CrossCoreWaitFlag<GROUP_CROSS_MODE, PIPE_V>(FLAG_O_READY);
#endif
                VectorStage2(oAccUBLocal, oDeltaUBLocal, alphaUBLocal, data);
                // Set 绑定生产 DONE 的 V, 确保 V2 完成后再上报.
#ifdef SIM_COMPATIBLE
                CrossCoreSetFlag<PAIR_CROSS_MODE, PIPE_V>(FLAG_DONE);
#else
                CrossCoreSetFlag<GROUP_CROSS_MODE, PIPE_V>(FLAG_DONE);
#endif
            }

            // 复用 PWork 完成归一化和 Cast, 然后写回 GM.
            asc_vf_call<FusedDivCastVF>(
                reinterpret_cast<__ubuf__ bfloat16_t *>(
                    pWorkUBLocal.GetPhyAddr()),
                reinterpret_cast<__ubuf__ float *>(oAccUBLocal.GetPhyAddr()),
                reinterpret_cast<__ubuf__ float *>(lUBLocal.GetPhyAddr()),
                static_cast<uint16_t>(halfBr), static_cast<uint16_t>(d));
            SetWaitFlag<HardEvent::V_MTE3>(STATIC_EVENT_ID1);
            DataCopy(outGlobal[outGMOffset], pWorkUBLocal, outputHalfElements);
            // 等待 UB -> GM 读取完成, 再允许下一 task 覆写 PWork.
