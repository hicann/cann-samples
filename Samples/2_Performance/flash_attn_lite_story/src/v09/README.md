# FALite v09：preload(C1)=4，首个 C2 前预发射四个 C1

## 本版内容

v09 将上一版的预发射深度从 3 增至 4，同时将 L0C 结果槽从 3 增至 4。调度仍是连续的 C1/C2 与 V1/V2 循环，Vector 仍使用分开的 Softmax、Cast/Pack 和普通输出乘加。本文记 `R=CV_PIPELINE_SLOT_NUM=4`，`L0C_QUEUE_DEPTH=4`。
...
| 矩阵乘槽 | `mmadOpIdx%2`、`mmadOpIdx%4` | 前者选 L0A/B，后者选 L0C |
...
首个 C2 在 AIC 的 epoch 3 发射，之前已经发射 `min(R,kvTileCount)` 个 C1；首个 V2 在 AIV 的 epoch 4 发射，之前同样已有最多 R 个 V1。预发射不等于结果已经完成。

以不少于 6 个 item 的 task 为例：
...
| 5 | C1(5) → C2(2) | V1(4) → V2(1) |

同一行只是把相同数值的循环编号排在一起，不是共同时间点。跨核先后由数据通知决定，不能把 C1(j) 与 V2(j) 的 epoch 编号差解释为真实时延。

## AIC 缓冲与核内同步

L1 按 P、K、Q、V 顺序连续分配；所有地址均由 Host tiling 计算。固定规格下 L1 共 384 KiB，L0A/B 各 64 KiB，L0C 共 256 KiB。

| 资源 | 槽数 × 单槽大小 | Mutex ID | 生命周期 |
| --- | --- | --- | --- |
| K L1 | 2 × 32 KiB | 0～1 | C1 MTE2 写入，MTE1 读 K 后归还 |
| V L1 | 4 × 32 KiB | 2～5 | C1 MTE2 写入，保留到对应 C2 的 MTE1 读完 |
| Q L1 | 2 × 32 KiB | 6～7 | task 起步搬入；首个 C1 取得 MTE1 所有权，末个 C1 归还 |
| P L1 | 4 × 32 KiB | 无本地写入 Mutex | 两路 AIV 写入，由 P_READY 保护 AIC 读取 |
| L0A/B | 各 2 × 32 KiB | 8～9，每号联合管理 A/B | MTE1 写入 → Mmad 读取 → 下一次装载 |
| L0C | 4 × 64 KiB | 10～13 | Mmad 写入 → Fixpipe 读取 → 下一次矩阵乘 |
...
每路 AIV 的 UB 共 225.75 KiB，两路独立分配：
...
| alpha | 4 × 256 B | 按 j%R，V1 写后保留到同 item 的 V2 |
| m/l | 各 1 × 256 B | 各 64 个 FP32 值，按 item 顺序递推 |

`rowStatsUBElems=64` 是单份行统计长度，不是 alpha 的总长度。alpha LocalTensor 的元素数是 `R*rowStatsUBElems=256`，其容量为 1024 B；Host 的 UB 容量检查也乘 R。
...
| P_READY | 2～5 | AIV MTE3 → AIC MTE1 | 两路 P 已进入共享 L1 |
| O_DELTA_HANDOFF | 6～7 | AIV V → AIC FIX | DeltaO 槽可写（初始及 V2 后归还） |
| O_DELTA_HANDOFF | 6～7 | AIC FIX → AIV V | DeltaO 已写好 |
...
所有阶段先检查 j 的有效范围。若 `n=1`，只生成 item 0：AIC 在 epoch 0 发 C1、epoch 3 发 C2；AIV 在 epoch 1 发 V1、epoch 4 发 V2。其余轮次为空，不发送 P_READY，也不等待不存在的 item。固定双槽的初始归还与最终消费仍全部执行。
...
相对 [v08](../v08/README.md)，只有 `CV_PIPELINE_SLOT_NUM: 3→4` 与 `L0C_QUEUE_DEPTH: 3→4` 两个常量变化，Host/AIC/AIV 及同步封装的其余代码相同。V/P/alpha 容量、相关 Mutex/flag 编号和 Host 地址随常量推导，K/S/DeltaO/PWork、Q/Output、L0A/B 的双槽数不变。

v08→v09 同时增加 R 与 L0C 深度，版本差异表示整套配置的变化，不能单独归因于其中一个常量。[v11](../v11/README.md) 保持本版 4/4，只改变 Vector 通路，可用于比较基础与压缩 Vector 的实现。L0C 槽数是独立参数，算法不要求它等于 R。

## 流水示意与性能参考

![v09 连续调度示意图](../../images/pipeline/falite_v09_pipeline.png)

上半图展示较新 C1/V1 与较早 C2/V2 的允许重叠，包含填充和排空；下半图展示同 item 就绪依赖。epoch 是各核自己的循环编号，色块按源码和槽位约束定性排列。反向归还及核内 Mutex 未全部画出，图宽不能用于比较实际耗时。

![v09 上板流水截图](../../images/pipe_trace/falite_v09_pipe.png)

截图来自 `B=1,N=1,S=2048` 的完整 PipeTimeline trace，展示一个 Mix 核组，窗口为 `[44.312,84.312] μs`。它呈现各 Pipe 的忙区与空隙，不能把某个空隙直接命名为一个 CrossCore Wait 的精确时长。

按[总文档的统一性能口径](../../README.md#性能模型与验证口径)，本版 `B=1,N=1,S=131072`、32 个 AIC 的 Kernel 耗时为 15120.490234 μs，有效 Cube MFU 为 67.3308%。完整比较见[统一性能结果](../../README.md#统一性能结果)。
...
cmake --build build --target falite_v09 -j
./build/Samples/2_Performance/flash_attn_lite_story/falite_v09 --core-num 1 --size 1 1 513
```
