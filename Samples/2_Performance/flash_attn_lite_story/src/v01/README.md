# FALite v01：多个 Mix 核组并行处理 task

## 本版内容

v01 把 v00 的 Query 分块任务分配给多个 Mix 核组。每组仍使用一套片上工作区，S、P、DeltaO 仍经过 GM；本版增加的是 task 级并行，核组内部没有增加双缓冲。
...
![v01 task 分核示意图](../../images/alg/falite_v01_task_partition.png)

图中 `B=2,N=2,S=640`，20 个 task 分给 4 个 Mix 核组；Mix 0 处理 `0/4/8/12/16`。颜色仅表示归属。
...
![v01 流水示意图](../../images/pipeline/falite_v01_pipeline.png)

示意图说明本版多核组 task 归属与组内 GM 交接的关系。色块宽度和图中空隙不代表实际耗时；跨 Pipe 的精确完成顺序应结合源码同步判断。

![v01 上板 PipeTimeline](../../images/pipe_trace/falite_v01_pipe.png)

截图来自 `B=1,N=1,S=2048,D=128` 的完整 PipeTimeline trace，窗口为 `[232.419,272.419] μs`。它展示各 Pipe 的忙区和空隙，不能把某个空隙直接解释为 DONE 或 P_READY 的精确等待时长。

总文档记录本版在 `B=1,N=1,S=131072`、32 个 AIC 下的 Task Duration 中位数为 `88377.640625 μs`。完整采集环境、核数差异和 MFU 口径统一见[总 README 的统一性能结果](../../README.md#统一性能结果)。本文不按示意图的宽度比较版本收益。
...
cmake --build build --target falite_v01 -j
./build/Samples/2_Performance/flash_attn_lite_story/falite_v01 --core-num 2 --size 2 3 257
```
...
相对 [v00](../v00/README.md)，v01 只增加 Host 核数选择、useAicNum 和各核心的固定步长 task 循环；矩阵乘、Vector 计算和 SRAM 布局不变。[v02](../v02/README.md) 则移除 S/DeltaO 的 GM 往返，仍不增加槽数。
