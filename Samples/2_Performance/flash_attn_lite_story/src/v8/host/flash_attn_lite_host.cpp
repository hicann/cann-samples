    ...
    uint32_t batchSize, uint32_t seqLen, float scale, uint32_t aicoreNum, FALite::FlashAttnLiteTilingData& data)
{
    ...
    // P/K/Q/V 在 L1 中连续排布.
    aic.pL1Addr = 0;
    aic.pL1Elems = FALite::CV_PIPELINE_SLOT_NUM * data.br * data.bc;
    aic.kL1Addr = aic.pL1Addr + aic.pL1Elems * sizeof(uint16_t);
    aic.kL1Elems = FALite::DB_SLOT_NUM * data.bc * FALite::HEAD_DIM;
    aic.qL1Addr = aic.kL1Addr + aic.kL1Elems * sizeof(uint16_t);
    aic.qL1Elems = FALite::DB_SLOT_NUM * data.br * FALite::HEAD_DIM;
    aic.vL1Addr = aic.qL1Addr + aic.qL1Elems * sizeof(uint16_t);
    aic.vL1Elems = FALite::CV_PIPELINE_SLOT_NUM * data.bc * FALite::HEAD_DIM;

    // L0A/L0B 是 MTE1->Mmad 的双缓冲, L0C 是独立的 Mmad->Fix 结果队列.
    aic.aL0AAddr = 0;
    aic.aL0AElems = FALite::DB_SLOT_NUM * data.bc * (data.br > FALite::HEAD_DIM ? data.br : FALite::HEAD_DIM);
    aic.bL0BAddr = 0;
    aic.bL0BElems = FALite::DB_SLOT_NUM * FALite::HEAD_DIM * (data.br > data.bc ? data.br : data.bc);
    aic.cL0CAddr = 0;
    aic.cL0CElems = FALite::L0C_QUEUE_DEPTH * data.br * (data.bc > FALite::HEAD_DIM ? data.bc : FALite::HEAD_DIM);

    auto& aiv = data.layoutAIV;
    const uint32_t halfBr = data.br / 2;
    aiv.sUBAddr = 0;
    aiv.sUBElems = FALite::DB_SLOT_NUM * halfBr * data.bc;
    aiv.oDeltaUBAddr = aiv.sUBAddr + aiv.sUBElems * sizeof(float);
    aiv.oDeltaUBElems = FALite::DB_SLOT_NUM * halfBr * FALite::HEAD_DIM;
    aiv.oAccUBAddr = aiv.oDeltaUBAddr + aiv.oDeltaUBElems * sizeof(float);
    aiv.oAccUBElems = FALite::DB_SLOT_NUM * halfBr * FALite::HEAD_DIM;
    ...
    aiv.pWorkUBElems = FALite::DB_SLOT_NUM * halfBr * (data.bc + 1);
    ...
    if (aic.vL1Addr + aic.vL1Elems * sizeof(uint16_t) > L1_CAPACITY_BYTES ||
        ...
        aiv.alphaUBAddr + FALite::CV_PIPELINE_SLOT_NUM * aiv.rowStatsUBElems * sizeof(float) > UB_CAPACITY_BYTES) {
