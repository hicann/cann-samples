...
constexpr MutexId MUTEX_V_L1_BASE = MUTEX_K_L1_BASE + DB_SLOT_NUM;
constexpr MutexId MUTEX_Q_L1_BASE = MUTEX_V_L1_BASE + CV_PIPELINE_SLOT_NUM;
constexpr MutexId MUTEX_L0AB_BASE = MUTEX_Q_L1_BASE + DB_SLOT_NUM;
constexpr MutexId MUTEX_L0C_BASE = MUTEX_L0AB_BASE + DB_SLOT_NUM;
    ...
    uint8_t dualDstCtl = 1)
{
    ...
    constexpr uint32_t FIXPIPE_N_ALIGN = C0ElemNum<srcT>();
...
// C1: K x Qᵀ -> Sᵀ. K 使用核内双槽, V 保留到同代 C2.
__aicore__ inline void CubeStage1(
    ...
    AscendC::LocalTensor<bfloat16_t>& bL0BLocal, AscendC::LocalTensor<float>& mmResultL0CLocal,
    ...
    uint32_t kSlot, uint32_t cvPipelineSlot, uint32_t sSlot, uint32_t l0abSlot, uint32_t l0cSlot, uint32_t ioSlot)
{
    using namespace AscendC;
    ...
    if ASCEND_IS_AIC {
        ...
        const MutexID kMutexId = MUTEX_K_L1_BASE + static_cast<MutexID>(kSlot);
        const MutexID vMutexId = MUTEX_V_L1_BASE + static_cast<MutexID>(cvPipelineSlot);
        const MutexID l0abMutexId = MUTEX_L0AB_BASE + static_cast<MutexID>(l0abSlot);
        const MutexID l0cMutexId = MUTEX_L0C_BASE + static_cast<MutexID>(l0cSlot);
        ...
        CubeMmad<float, bfloat16_t, bfloat16_t>(mmResultL0CLocal, aL0ALocal, bL0BLocal, bc, br, d, true);
        Mutex::Unlock<PIPE_M>(l0abMutexId);
        Mutex::Unlock<PIPE_M>(l0cMutexId);

        Mutex::Lock<PIPE_FIX>(l0cMutexId);
        WaitAivToAic<PIPE_FIX>(SlotFlagId(FLAG_S_HANDOFF_BASE, sSlot));
        FixpipeToVecUB<float, float>(sUBLocal, mmResultL0CLocal, bc, br, 2);
        SetAicToAiv<PIPE_FIX>(SlotFlagId(FLAG_S_HANDOFF_BASE, sSlot));
        Mutex::Unlock<PIPE_FIX>(l0cMutexId);
    }
}

// C2: P x V -> oDelta. P/V 按 cvPipelineSlot 与对应 C1/V1 配对.
__aicore__ inline void CubeStage2(
    ...
    AscendC::LocalTensor<float>& mmResultL0CLocal, AscendC::LocalTensor<float>& oDeltaUBLocal,
    const FlashAttnLiteTilingData& data, uint32_t cvPipelineSlot, uint32_t oDeltaSlot, uint32_t l0abSlot,
    uint32_t l0cSlot)
{
    using namespace AscendC;
    ...
    if ASCEND_IS_AIC {
        ...
        // 先等待 P ready, 避免在 P 尚未生成时提前占用 L0A/L0B.
        WaitAivToAic<PIPE_MTE1>(SlotFlagId(FLAG_P_READY_BASE, cvPipelineSlot));
        Mutex::Lock<PIPE_MTE1>(l0abMutexId);
        ...
        Mutex::Lock<PIPE_M>(l0abMutexId);
        Mutex::Lock<PIPE_M>(l0cMutexId);
        CubeMmad<float, bfloat16_t, bfloat16_t>(mmResultL0CLocal, aL0ALocal, bL0BLocal, br, d, bc, true);
        Mutex::Unlock<PIPE_M>(l0abMutexId);
        Mutex::Unlock<PIPE_M>(l0cMutexId);

        Mutex::Lock<PIPE_FIX>(l0cMutexId);
        WaitAivToAic<PIPE_FIX>(SlotFlagId(FLAG_O_DELTA_HANDOFF_BASE, oDeltaSlot));
        FixpipeToVecUB<float, float>(oDeltaUBLocal, mmResultL0CLocal, br, d);
        SetAicToAiv<PIPE_FIX>(SlotFlagId(FLAG_O_DELTA_HANDOFF_BASE, oDeltaSlot));
        Mutex::Unlock<PIPE_FIX>(l0cMutexId);
    }
}
    ...
    __gm__ bfloat16_t* qGMAddr, __gm__ bfloat16_t* kGMAddr, __gm__ bfloat16_t* vGMAddr, FlashAttnLiteTilingData data)
{
    ...
    if ASCEND_IS_AIC {
        ...
        const uint32_t aL0ATileElements = aic.aL0AElems / DB_SLOT_NUM;
        const uint32_t bL0BTileElements = aic.bL0BElems / DB_SLOT_NUM;
        const uint32_t cL0CTileElements = aic.cL0CElems / L0C_QUEUE_DEPTH;
        ...
        LocalTensor<float> mmResultL0CLocal(TPosition::CO1, aic.cL0CAddr, aic.cL0CElems);
        ...
        for (uint32_t taskId = GetBlockIdx(); taskId < data.numTasks; taskId += GetBlockNum()) {
            ...
            for (uint32_t epoch = 0; epoch < data.tc + CV_PIPELINE_SLOT_NUM; ++epoch) {
                if (epoch < data.tc) {
                    const uint32_t j = epoch;
                    const uint32_t kSlot = j % DB_SLOT_NUM;
                    const uint32_t cvPipelineSlot = j % CV_PIPELINE_SLOT_NUM;
                    const uint32_t sSlot = j % DB_SLOT_NUM;
                    const uint32_t l0abSlot = mmadOpIdx % DB_SLOT_NUM;
                    const uint32_t l0cSlot = mmadOpIdx % L0C_QUEUE_DEPTH;
                    auto kSlotL1Local = kL1Local[static_cast<uint64_t>(kSlot) * kTileElements];
                    auto vSlotL1Local = vL1Local[static_cast<uint64_t>(cvPipelineSlot) * vTileElements];
                    auto aSlotL0ALocal = aL0ALocal[static_cast<uint64_t>(l0abSlot) * aL0ATileElements];
                    auto bSlotL0BLocal = bL0BLocal[static_cast<uint64_t>(l0abSlot) * bL0BTileElements];
                    auto cSlotL0CLocal = mmResultL0CLocal[static_cast<uint64_t>(l0cSlot) * cL0CTileElements];
                    auto sSlotUBLocal = sUBLocal[static_cast<uint64_t>(sSlot) * sHalfTileElements];
                    CubeStage1(
                        qSlotL1Local, kSlotL1Local, vSlotL1Local, aSlotL0ALocal, bSlotL0BLocal, cSlotL0CLocal,
                        sSlotUBLocal, kGlobal, vGlobal, data, j, batchIdx, kSlot, cvPipelineSlot, sSlot, l0abSlot,
                        l0cSlot, ioSlot);
                    ++mmadOpIdx;
                }

                if (epoch + 1 >= CV_PIPELINE_SLOT_NUM) {
                    const uint32_t j = epoch - CV_PIPELINE_SLOT_NUM + 1;
                    if (j < data.tc) {
                        const uint32_t cvPipelineSlot = j % CV_PIPELINE_SLOT_NUM;
                        const uint32_t oDeltaSlot = j % DB_SLOT_NUM;
                        const uint32_t l0abSlot = mmadOpIdx % DB_SLOT_NUM;
                        const uint32_t l0cSlot = mmadOpIdx % L0C_QUEUE_DEPTH;
                        auto pSlotL1Local = pL1Local[static_cast<uint64_t>(cvPipelineSlot) * pTileElements];
                        auto vSlotL1Local = vL1Local[static_cast<uint64_t>(cvPipelineSlot) * vTileElements];
                        auto aSlotL0ALocal = aL0ALocal[static_cast<uint64_t>(l0abSlot) * aL0ATileElements];
                        auto bSlotL0BLocal = bL0BLocal[static_cast<uint64_t>(l0abSlot) * bL0BTileElements];
                        auto cSlotL0CLocal = mmResultL0CLocal[static_cast<uint64_t>(l0cSlot) * cL0CTileElements];
                        auto oDeltaSlotUBLocal =
                            oDeltaUBLocal[static_cast<uint64_t>(oDeltaSlot) * oDeltaHalfTileElements];
                        CubeStage2(
                            pSlotL1Local, vSlotL1Local, aSlotL0ALocal, bSlotL0BLocal, cSlotL0CLocal, oDeltaSlotUBLocal,
                            data, cvPipelineSlot, oDeltaSlot, l0abSlot, l0cSlot);
                        ++mmadOpIdx;
                    }
                }
            }
        ...
        for (uint32_t slot = 0; slot < DB_SLOT_NUM; ++slot) {
            WaitAivToAic<PIPE_FIX>(SlotFlagId(FLAG_S_HANDOFF_BASE, slot));
        }
        for (uint32_t slot = 0; slot < DB_SLOT_NUM; ++slot) {
            WaitAivToAic<PIPE_FIX>(SlotFlagId(FLAG_O_DELTA_HANDOFF_BASE, slot));
        }
    } // ASCEND_IS_AIC
}
