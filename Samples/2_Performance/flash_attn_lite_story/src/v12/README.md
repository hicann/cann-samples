# FALite v12：五代 V/P/alpha 配合四槽 L0C
...
本版 `R=CV_PIPELINE_SLOT_NUM=5`、`L0C_DEPTH=L0C_QUEUE_DEPTH=4`。`epoch` 是每颗核心自己的循环编号，不是跨核共同时刻。AIC 每轮先 C1、后 C2；AIV 每轮先 V1、后 V2。仅当 item 存在时才发射：

```text
AIC 本地：
  epoch 0～3：依次 C1(0)～C1(3)
  epoch 4：C1(4) → C2(0)
  epoch 5：C1(5) → C2(1)
  一般轮 e：C1(e) → C2(e-4)
...
AIV 本地：
  epoch 0：无阶段
  epoch 1～4：依次 V1(0)～V1(3)
  epoch 5：V1(4) → V2(0)
  epoch 6：V1(5) → V2(1)
  一般轮 e：V1(e-1) → V2(e-5)
```

因此长 task 的首个 C2 前已发射 5 个 C1，首个 V2 前已发射 5 个 V1。发射不等于完成，AIC 与两路 AIV 分别在各自的 CrossCore 消费点等待数据。短 task 和排空时，越界项直接跳过。
...
| `j%5` | V L1、P L1、alpha UB | 保存跨 C1→C2、V1→V2 的 5 代数据 |
...
| P，P_READY | 2～6 | AIV PIPE_MTE3 → AIC PIPE_MTE1 | 无独立 free |
| DeltaO，O_DELTA_HANDOFF | 7～8 | AIC PIPE_FIX → AIV PIPE_V | AIV PIPE_V → AIC PIPE_FIX |
...
| V L1 | 5 × 32 KiB | 2～6 | 同 item 的 C2 MTE1 读完 V |
| Q L1 | 2 × 32 KiB | 7～8 | task 最后一个有效 C1 装完 Q |
| P L1 | 5 × 32 KiB | CrossCore 管理 | 同 item 的 C2 读完 P |
| L0A、L0B | 各 2 × 32 KiB | 9～10，每号联合管理 A/B | Mmad 读完该对输入 |
| L0C | 4 × 64 KiB | 11～14 | Fixpipe 读完该结果 |

L1 共 448 KiB，L0A/L0B 各 64 KiB，L0C 256 KiB。P 与 V 虽然都按 `j%5` 编号，仍位于不同物理地址。
...
每路 UB 共 226 KiB，两路配置相同但各自拥有独立状态和 Mutex。
...
| alpha | 5 × 256 B | 保存到对应 V2 |

`rowStatsUBElems=64` 表示一份行状态的元素数；alpha 的 LocalTensor 总长度为 `R*rowStatsUBElems=320`，单槽偏移仍是 `(j%R)*64`。不要把 rowStatsUBElems 当作 alpha 全部槽长度。
...
与 [v11](../v11/README.md) 相比，源码只改变 `CV_PIPELINE_SLOT_NUM:4→5`。Host 地址、V/P/alpha 槽数、相关 Mutex 与 flag 编号随常量推导；L0C_QUEUE_DEPTH 仍为 4，压缩 Vector 与所有阶段实现相同。

额外一代 V/P 各占 32 KiB L1，每路 alpha 多占 256 B UB，所以 L1 从 384 增至 448 KiB，单路 UB 从 225.75 增至 226 KiB。这一对照改变预发射深度及其配套存储，保持 L0C 不变；它不等同于 v10→v11 的 R/L0C 双变量配置对照。

L0C 没有第五槽：一个 FP32 128×128 结果占 64 KiB，五槽需 320 KiB，超过 256 KiB 容量。首五个 C1 的 L0C 编号为 `0,1,2,3,0`；第五次写入必须等第一槽的 Fixpipe 消费完成。V/P 保存五代与 L0C 四槽并不矛盾，它们的生命周期不同。
