# FALite v11：四槽配置下的压缩 Vector 通路

## 本版内容

v11 保持 v09 的 `R=4,L0C=4`，把基础 Vector 通路改为与 v10 相同的压缩写法。四槽配置不变，重点是 V1 直接生成 BF16 NZ P、首轮覆盖以及四组寄存器的输出更新。
...
本版 `R=CV_PIPELINE_SLOT_NUM=4`、`L0C_DEPTH=L0C_QUEUE_DEPTH=4`。`epoch` 是每颗核心自己的循环编号，不是跨核共同时刻。AIC 每轮先 C1、后 C2；AIV 每轮先 V1、后 V2。仅当 item 存在时才发射：

```text
AIC 本地：
  epoch 0～2：依次 C1(0)～C1(2)
  epoch 3：C1(3) → C2(0)
  epoch 4：C1(4) → C2(1)
  一般轮 e：C1(e) → C2(e-3)
...
AIV 本地：
  epoch 0：无阶段
  epoch 1～3：依次 V1(0)～V1(2)
  epoch 4：V1(3) → V2(0)
  epoch 5：V1(4) → V2(1)
  一般轮 e：V1(e-1) → V2(e-4)
```

因此长 task 的首个 C2 前已发射 4 个 C1，首个 V2 前已发射 4 个 V1。发射不等于完成，AIC 与两路 AIV 分别在各自的 CrossCore 消费点等待数据。短 task 和排空时，越界项直接跳过。
...
| `j%4` | V L1、P L1、alpha UB | 保存跨 C1→C2、V1→V2 的 4 代数据 |
| `mmadOpIdx%2` | L0A/L0B | C1/C2 共用的矩阵乘发射总序号 |
| `mmadOpIdx%4` | L0C | 同一总序号，独立结果队列深度 |
...
| P，P_READY | 2～5 | AIV PIPE_MTE3 → AIC PIPE_MTE1 | 无独立 free |
| DeltaO，O_DELTA_HANDOFF | 6～7 | AIC PIPE_FIX → AIV PIPE_V | AIV PIPE_V → AIC PIPE_FIX |
...
| V L1 | 4 × 32 KiB | 2～5 | 同 item 的 C2 MTE1 读完 V |
| Q L1 | 2 × 32 KiB | 6～7 | task 最后一个有效 C1 装完 Q |
| P L1 | 4 × 32 KiB | CrossCore 管理 | 同 item 的 C2 读完 P |
| L0A、L0B | 各 2 × 32 KiB | 8～9，每号联合管理 A/B | Mmad 读完该对输入 |
| L0C | 4 × 64 KiB | 10～13 | Fixpipe 读完该结果 |

L1 共 384 KiB，L0A/L0B 各 64 KiB，L0C 256 KiB。P 与 V 虽然都按 `j%4` 编号，仍位于不同物理地址。
...
每路 UB 共 225.75 KiB，两路配置相同但各自拥有独立状态和 Mutex。
...
| alpha | 4 × 256 B | 保存到对应 V2 |

`rowStatsUBElems=64` 表示一份行状态的元素数；alpha 的 LocalTensor 总长度为 `R*rowStatsUBElems=256`，单槽偏移仍是 `(j%R)*64`。不要把 rowStatsUBElems 当作 alpha 全部槽长度。
...
## 从 v09 到 v11，以及与 v10 的区别

[v09](../v09/README.md) 与 v11 的 Host、AIC、公共参数、槽位和 CrossCore 协议相同，只有 AIV 的 Vector 通路不同：V1 合并 Softmax 与 Cast/Pack，首轮直接建立状态，后续使用多路寄存器和 `MulDstAdd`。这是四槽配置下的直接对照；三槽下对应 v08→v10。

[v10](../v10/README.md) 与 v11 的源码差异只有 `CV_PIPELINE_SLOT_NUM:3→4` 和 `L0C_QUEUE_DEPTH:3→4`。L1 从 320 增至 384 KiB，L0C 从 192 增至 256 KiB，UB 从 225.50 增至 225.75 KiB，比较的是整套配置，不能单独归因于 R。

[v12](../v12/README.md) 保持压缩 Vector 和四槽 L0C，只把 V/P/alpha 的保存代数从 4 增至 5。

## 流水示意与性能参考

![v11 连续滚动流水示意图](../../images/pipeline/falite_v11_pipeline.png)

示意图包含填充、滚动和排空，上半图展示允许的重叠，下半图展示同 item 的就绪依赖；省略反向归还和核内 Mutex。图中的 epoch 是本核循环编号，色块宽度不是实测耗时。

![v11 上板流水截图](../../images/pipe_trace/falite_v11_pipe.png)

PipeTimeline 截图使用 `B=1,N=1,S=2048`、单 Mix 组，窗口为 `[39.165,79.165]` μs。

长序列统一口径下，本版 Task Duration 为 10618.893555 μs，因果有效 Cube MFU 为 95.8738%；条件及完整比较见[总文档性能表](../../README.md#统一性能结果)。
...
从仓库根目录可构建 `cmake --build build --target falite_v11 -j`，运行 `./build/Samples/2_Performance/flash_attn_lite_story/falite_v11 --core-num 1 --size 1 1 513`，覆盖不足 R 的短 task、首次 V/P/alpha 回卷及序列尾块。完整配置与精度标准见[编译、运行与复现](../../README.md#编译运行与复现)。
