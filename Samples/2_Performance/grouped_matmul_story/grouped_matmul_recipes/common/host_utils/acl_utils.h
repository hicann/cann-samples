...
class AclDeviceBuffers {
public:
    ~AclDeviceBuffers()
    {
        for (void* deviceBuffer : deviceBuffers_) {
            (void)aclrtFree(deviceBuffer);
        }
    }

    void Push(void* deviceBuffer)
    {
        if (deviceBuffer != nullptr) {
            deviceBuffers_.push_back(deviceBuffer);
        }
    }

    AclDeviceBuffers() = default;
    AclDeviceBuffers(const AclDeviceBuffers&) = delete;
    AclDeviceBuffers& operator=(const AclDeviceBuffers&) = delete;
