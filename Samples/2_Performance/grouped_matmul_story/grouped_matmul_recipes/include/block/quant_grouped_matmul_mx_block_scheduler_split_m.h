...
public:
    ...
    __aicore__ inline void UpdateTailTile()
    {
        ...
        int64_t mMin = Min(static_cast<int64_t>(AscendC::BLOCK_CUBE), static_cast<int64_t>(mTailAlign_));
        int64_t nMin = Min(static_cast<int64_t>(AscendC::BLOCK_CUBE), static_cast<int64_t>(nTailAlign_));
    ...
    __aicore__ inline TupleShape GetBlockShape(const BlockCoord& blockCoord)
    {
        ...
        singleCoreMSplit = Align(singleCoreMSplit, mTailAlign_);
