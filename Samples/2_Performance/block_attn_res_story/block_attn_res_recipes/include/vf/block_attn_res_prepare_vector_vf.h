...
__simd_vf__ inline void AccumulateSquareDot(
    __ubuf__ float* qAddr, __ubuf__ float* vAddr, __ubuf__ float* statAddr, uint32_t validD, uint32_t nIndex)
{
    ...
    StoreAlign<float, StoreDist::DIST_FIRST_ELEMENT_B32>(
        statAddr + SUM_SQUARE_OFFSET + nIndex, squareReduceReg, oneMask);
...
__simd_vf__ inline void WeightedAccumulate(
    __ubuf__ float* vAddr, __ubuf__ float* weightAddr, __ubuf__ float* outputAddr, uint32_t validD)
{
    RegTensor<float> vReg;
