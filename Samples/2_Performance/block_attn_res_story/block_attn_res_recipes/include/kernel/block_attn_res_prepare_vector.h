...
#include "vf/block_attn_res_prepare_vector_vf.h"
...
private:
    ...
    __aicore__ inline static auto MakeGmTensor(__gm__ T* address, int64_t rows, int64_t columns)
    {
        return AscendC::Te::MakeTensor(
            ...
            AscendC::Te::MakeFrameLayout<AscendC::Te::NDExtLayoutPtn, AscendC::Te::LayoutTraitDefault<T>>(
                rows, columns));
    }

    template <typename T, typename GmTensor>
    __aicore__ inline static void CopyGmToUb(
        uint64_t dstByteOffset, const GmTensor& srcTensor, int64_t rowIndex, int64_t columnIndex, int64_t validElements,
        int64_t ubPitch)
    {
    ...
    __aicore__ inline static void CopyUbToGm(
        GmTensor& dstTensor, int64_t rowIndex, int64_t columnIndex, uint64_t srcByteOffset, int64_t validElements,
        int64_t ubPitch)
    {
        auto gmTensor = dstTensor.Slice(
            AscendC::Te::MakeCoord(rowIndex, columnIndex),
            AscendC::Te::MakeShape(static_cast<int64_t>(1), validElements));
        auto ubStorageTensor = AscendC::Te::MakeTensor(
    ...
    __aicore__ inline __ubuf__ float* CopyInV(
        uint64_t tIndex, uint32_t nIndex, uint32_t dOffset, uint32_t validD, uint64_t vLoop)
    {
    ...
    __aicore__ inline __ubuf__ float* CopyVToCache(
        uint64_t tIndex, uint32_t nIndex, uint32_t dOffset, uint32_t cacheOffset, uint32_t validD)
    {
    ...
    __aicore__ inline void CopyOutStats(uint64_t sIndex, uint64_t tIndex, uint64_t statByteOffset, event_t eventId)
    {
        ...
        CopyUbToGm<float>(
            logitMaxTensor, static_cast<int64_t>(sIndex), static_cast<int64_t>(tIndex),
            statByteOffset + static_cast<uint64_t>(BlockAttnResPrepareVF::MAX_OFFSET) * FP32_BYTES, 1,
            FP32_32B_ALIGN_ELEMS);
        CopyUbToGm<float>(
            expSumTensor, static_cast<int64_t>(sIndex), static_cast<int64_t>(tIndex),
    ...
    __aicore__ inline void AccumulateStats(
        __ubuf__ float* qAddr, __ubuf__ float* vAddr, __ubuf__ float* statAddr, uint32_t dLoop, uint32_t validD,
        uint32_t nIndex)
    {
        if (dLoop == 0U) {
    ...
    __aicore__ inline void AccumulateOutput(
        __ubuf__ float* vAddr, __ubuf__ float* weightAddr, __ubuf__ float* outputAddr, uint32_t nIndex, uint32_t validN,
        uint32_t validD)
    {
        if (validN == 1U) {
    ...
    __aicore__ inline __ubuf__ float* AcquireOutputTile(
        uint32_t dLoop, uint32_t& dOffset, uint32_t& validD, event_t& outputEvent)
    {
        dOffset = dLoop * tilingData_->baseD;
        const uint64_t remainingD = tilingData_->totalD - dOffset;
        validD = static_cast<uint32_t>(remainingD < tilingData_->baseD ? remainingD : tilingData_->baseD);
        ...
        outputEvent = static_cast<event_t>(outputBufferIndex);
        wait_flag(PIPE_MTE3, PIPE_V, outputEvent);
        return GetUbAddr<float>(outputByteOffset);
    }
    ...
    __aicore__ inline void ProcessEmptyWork(uint64_t workId)
    {
        ...
        for (uint32_t dLoop = 0; dLoop < dTileNum_; ++dLoop) {
            uint32_t dOffset, validD;
            event_t outputEvent;
            __ubuf__ float* outputAddr = AcquireOutputTile(dLoop, dOffset, validD, outputEvent);
    ...
    __aicore__ inline void ProcessOutput(
        uint64_t sIndex, uint64_t tIndex, uint32_t validN, bool cacheAllV, __ubuf__ float* statAddr)
    {
    ...
    __aicore__ inline void ProcessWork(uint64_t workId, uint32_t validN, bool cacheAllV)
    {
        ...
        ProcessOutput(sIndex, tIndex, validN, cacheAllV, statAddr);
