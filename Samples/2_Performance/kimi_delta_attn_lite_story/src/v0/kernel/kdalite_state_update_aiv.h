...
constexpr uint32_t STATE_VALUE_UB_ADDR = STATE_RESULT_UB_ADDR + STATE_RESULT_UB_ELEMS * sizeof(float);
...
__simd_vf__ inline void UpdateStateVF(__ubuf__ float* state, __ubuf__ float* stateDelta, __ubuf__ float* gLast)
{
    ...
    for (uint16_t row = 0; row < HEAD_DIM; ++row) {
        ...
        Reg::LoadAlign(stateReg, state + row * AIV_DV_TILE);
        Reg::LoadAlign(deltaReg, stateDelta + row * AIV_DV_TILE);
        Reg::MulDstAdd(stateReg, decayReg, deltaReg, first16);
        Reg::StoreAlign<float, Reg::StoreDist::DIST_NORM_B32>(state + row * AIV_DV_TILE, stateReg, first16);
    }
}

__aicore__ inline void KernelProcessStateUpdateForAIV(
    __gm__ uint8_t* finalStateGMAddr, __gm__ uint8_t* workspaceGMAddr, const KimiDeltaAttnLiteTilingData& data)
{
    using namespace AscendC;

    if ASCEND_IS_AIV {
        GlobalTensor<float> uGlobal, gLastGlobal, finalStateGlobal;
        ...
        finalStateGlobal.SetGlobalBuffer(reinterpret_cast<__gm__ float*>(finalStateGMAddr));
        ...
        for (uint32_t taskId = aicIdx; taskId < data.stateNumTasks; taskId += data.stateUseAicNum) {
            ...
            for (uint32_t chunkId = 0; chunkId < data.chunkCount; ++chunkId) {
                ...
                Cast(stateShadowUBLocal, stateUBLocal, RoundMode::CAST_RINT, HEAD_DIM * AIV_DV_TILE);
                ...
                Sub(rWorkFP32UBLocal, rWorkFP32UBLocal, resultUBLocal, CHUNK_SIZE * AIV_DV_TILE);
                ...
                asc_vf_call<UpdateStateVF>(
                    reinterpret_cast<__ubuf__ float*>(stateUBLocal.GetPhyAddr()),
                    reinterpret_cast<__ubuf__ float*>(resultUBLocal.GetPhyAddr()),
