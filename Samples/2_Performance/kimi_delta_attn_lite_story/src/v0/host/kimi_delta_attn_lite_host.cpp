...
constexpr uint64_t W_BYTES_PER_CHUNK = KDALite::CHUNK_SIZE * KDALite::HEAD_DIM * sizeof(uint16_t);
constexpr uint64_t Q_PLUS_BYTES_PER_CHUNK = W_BYTES_PER_CHUNK;
constexpr uint64_t K_TAIL_BYTES_PER_CHUNK = W_BYTES_PER_CHUNK;
constexpr uint64_t A_BYTES_PER_CHUNK = KDALite::CHUNK_SIZE * KDALite::CHUNK_SIZE * sizeof(uint16_t);
constexpr uint64_t U_BYTES_PER_CHUNK = KDALite::CHUNK_SIZE * KDALite::VALUE_DIM * sizeof(float);
constexpr uint64_t R_BYTES_PER_CHUNK = KDALite::CHUNK_SIZE * KDALite::VALUE_DIM * sizeof(uint16_t);
constexpr uint64_t G_LAST_BYTES_PER_CHUNK = KDALite::HEAD_DIM * sizeof(float);
constexpr uint64_t O_HISTORY_BYTES_PER_CHUNK = KDALite::CHUNK_SIZE * KDALite::VALUE_DIM * sizeof(float);
    ...
    uint32_t batchSize, uint32_t seqLen, uint32_t mixCoreNum, KDALite::KimiDeltaAttnLiteTilingData& data)
{
    ...
    data.outputNumTasks = static_cast<uint32_t>(outputTasks);
    data.prepareUseAicNum = data.prepareNumTasks < mixCoreNum ? data.prepareNumTasks : mixCoreNum;
    data.stateUseAicNum = data.stateNumTasks < mixCoreNum ? data.stateNumTasks : mixCoreNum;
    data.outputUseAicNum = data.outputNumTasks < mixCoreNum ? data.outputNumTasks : mixCoreNum;

    uint64_t cursor = 0;
    if (!AppendChunkWorkspaceSegment(totalChunks, W_BYTES_PER_CHUNK, cursor, data.wOffset) ||
        !AppendChunkWorkspaceSegment(totalChunks, Q_PLUS_BYTES_PER_CHUNK, cursor, data.qPlusOffset) ||
        !AppendChunkWorkspaceSegment(totalChunks, K_TAIL_BYTES_PER_CHUNK, cursor, data.kTailOffset) ||
        !AppendChunkWorkspaceSegment(totalChunks, A_BYTES_PER_CHUNK, cursor, data.aOffset) ||
        !AppendChunkWorkspaceSegment(totalChunks, U_BYTES_PER_CHUNK, cursor, data.uOffset) ||
        !AppendChunkWorkspaceSegment(totalChunks, R_BYTES_PER_CHUNK, cursor, data.rOffset) ||
        !AppendChunkWorkspaceSegment(totalChunks, G_LAST_BYTES_PER_CHUNK, cursor, data.gLastOffset) ||
        !AppendChunkWorkspaceSegment(totalChunks, O_HISTORY_BYTES_PER_CHUNK, cursor, data.oHistoryOffset)) {
    ...
    aclrtStream stream)
{
    ...
    std::printf(
        "kdalite: 请求启动 kernel，Prepare=%u Mix，StateUpdate=%u Mix，LocalOutput=%u Mix，"
        "workspace=%llu bytes\n",
        data.prepareUseAicNum, data.stateUseAicNum, data.outputUseAicNum,
