    ...
    uint32_t batchSize, uint32_t seqLen, uint32_t mixCoreNum, KDALite::KimiDeltaAttnLiteTilingData& data)
{
    ...
    data.preparePairNumTasks = static_cast<uint32_t>((totalChunks + 1) / 2);
    data.prepareUseAicNum = data.preparePairNumTasks < mixCoreNum ? data.preparePairNumTasks : mixCoreNum;
    ...
    const uint32_t tasksPerUsedAic = data.stateNumTasks / data.stateUseAicNum;
    data.statePipelineLaneCount = tasksPerUsedAic >= 4 ? 4 : (tasksPerUsedAic >= 2 ? 2 : 1);

    uint64_t cursor = 0;
    if (!AppendChunkWorkspaceSegment(totalChunks, CHUNK_D_BF16_BYTES, cursor, data.wOffset) ||
