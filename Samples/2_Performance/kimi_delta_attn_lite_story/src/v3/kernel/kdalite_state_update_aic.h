    ...
    bool preloadStatic)
{
    ...
    if (preloadStatic) {
        PreloadStaticInputs(
            ...
            aGlobal, chunkIndex * CHUNK_D_ELEMS, chunkIndex * CHUNK_C_ELEMS, staticMutexId);
    }
...
__aicore__ inline void IssueStateC2History(
    ...
    uint32_t staticSlot, uint32_t itemId, uint32_t& l0aOpIdx)
{
    using namespace AscendC;
...
__aicore__ inline void IssueStateC2Remainder(
    ...
    AscendC::LocalTensor<float>& outputL0CLocal, uint32_t staticSlot, uint32_t itemId, uint32_t rQueueDepth,
        ...
        for (uint64_t waveTaskBase = static_cast<uint64_t>(aicIdx) * configuredLaneCount;
            ...
            if (laneCount == 1) {
                ...
                IssueStateInputPreload(
                    wGlobal, qPlusGlobal, mGlobal, kTailGlobal, aGlobal, valueGlobal, staticL1Local, valueL1Local, data,
                    taskId, 0, 0, 0, true);
                IssueStateU(
                ...
                for (uint32_t itemId = 0; itemId < itemCount; ++itemId) {
                    ...
                    if (nextItemId < itemCount) {
                        ...
                        IssueStateInputPreload(
                            wGlobal, qPlusGlobal, mGlobal, kTailGlobal, aGlobal, valueGlobal, staticL1Local,
                            valueL1Local, data, taskId, nextStaticSlot, nextItemId, nextItemId, true);
                    }

                    IssueStateC1Post(
                        staticL1Local, stateL1Local, aL0ALocal, stateL0BLocal, resultL0CLocal, predUBLocal, 0,
                        staticSlot, itemId, l0aOpIdx, l0cOpIdx);
                    IssueStateC2History(
                        staticL1Local, aL0ALocal, stateL0BLocal, outputL0CLocal, 0, staticSlot, itemId, l0aOpIdx);
                    ...
                    IssueStateC2Remainder(
                        staticL1Local, rL1Local, aL0ALocal, rL0BLocal, resultL0CLocal, deltaUBLocal, outputL0CLocal,
                ...
                for (uint32_t epoch = 0; epoch < epochCount; ++epoch) {
                    ...
                    if (hasC2) {
                        ...
                        const uint32_t staticSlot = chunkId % DB_SLOT_NUM;
                        IssueStateC2History(
                            staticL1Local, aL0ALocal, stateL0BLocal, outputL0CLocal, lane, staticSlot, itemId,
                            l0aOpIdx);
                        IssueStateC2Remainder(
                            staticL1Local, rL1Local, aL0ALocal, rL0BLocal, resultL0CLocal, deltaUBLocal, outputL0CLocal,
                    ...
                    if (hasC1) {
                        ...
                        const uint32_t staticSlot = chunkId % DB_SLOT_NUM;
                        IssueStateC1Post(
                            staticL1Local, stateL1Local, aL0ALocal, stateL0BLocal, resultL0CLocal, predUBLocal, lane,
                            staticSlot, itemId, l0aOpIdx, l0cOpIdx);
                    }
                    if (hasC2) {
                        const uint32_t itemId = epoch - laneCount + 1;
