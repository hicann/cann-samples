...
constexpr uint32_t STATE_AIV_UB_END_ADDR = STATE_DECAY_UB_ADDR + HEAD_DIM * sizeof(float) * STATE_PIPELINE_MAX_LANE_NUM;
...
static_assert(MUTEX_DECAY_UB_BASE + STATE_PIPELINE_MAX_LANE_NUM - 1 <= 27, "StateOutput AIV MutexID exceeds 27");
...
__aicore__ inline void IssueStateV2(
    AscendC::LocalTensor<bfloat16_t>& stateL1Local, AscendC::LocalTensor<float>& deltaUBLocal,
    AscendC::LocalTensor<float>& stateUBLocal, AscendC::LocalTensor<bfloat16_t>& stateShadowUBLocal,
    AscendC::LocalTensor<float>& stateDecayUBLocal, const KimiDeltaAttnLiteTilingData& data, uint32_t lane,
    uint32_t chunkId, uint32_t itemId, uint32_t subAivIdx)
{
    using namespace AscendC;
    ...
    WaitAicToAiv<PIPE_V>(deltaFlagId);
    asc_vf_call<UpdateStateAndShadowVF>(
        ...
        reinterpret_cast<__ubuf__ float*>(deltaUBLocal[deltaSlot * STATE_HALF_ELEMS].GetPhyAddr()),
        reinterpret_cast<__ubuf__ float*>(decaySlotUBLocal.GetPhyAddr()));
    SetAivToAic<PIPE_V>(deltaFlagId);
        ...
        for (uint64_t waveTaskBase = static_cast<uint64_t>(aicIdx) * configuredLaneCount;
            ...
            for (uint32_t epoch = 0; epoch < epochCount; ++epoch) {
                if (epoch >= laneCount && epoch < itemCount + laneCount) {
                    ...
                    IssueStateV2(
                        stateL1Local, deltaUBLocal, stateUBLocal, stateShadowUBLocal, stateDecayUBLocal, data, lane,
                        chunkId, itemId, subAivIdx);
                }
