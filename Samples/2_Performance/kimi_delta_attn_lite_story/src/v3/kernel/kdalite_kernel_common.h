...
// StateOutput 的多 lane 路径使用 state, U/prediction, R 和 delta 四组 CrossCore 交接.
// state 按物理 lane 分配 4 个 FlagID, R 队列最多使用 3 个 FlagID. Ascend 950 的逻辑
// FlagID 只能取 0..10.
...
constexpr uint16_t FLAG_DELTA_HANDOFF_BASE = FLAG_R_HANDOFF_BASE + STATE_R_MAX_QUEUE_DEPTH;
// wave 结束前 state lane 0 通道已排空, 可复用其 FlagID 传递 wave 完成信号.
constexpr uint16_t FLAG_STATE_WAVE_DONE = FLAG_STATE_HANDOFF_BASE;
static_assert(FLAG_DELTA_HANDOFF_BASE + DB_SLOT_NUM - 1 <= 10, "StateOutput multi-lane CrossCore FlagID exceeds 10");

// 单 lane 路径使用独立的 R 和 delta 交接协议.
...
static_assert(
    FLAG_SINGLE_LANE_DELTA_HANDOFF_BASE + DB_SLOT_NUM - 1 <= 10, "StateOutput single-lane CrossCore FlagID exceeds 10");
...
// Prepare 用两个 CV 时间槽滚动发射 VP/Cpair/VS/Cw. 每个时间槽含两路
// AIV 子槽, 每个子槽先保存 factor, 再分时保存 M/KPlus.
...
// Fixpipe 结果位于每路 AIV 的本地 UB. U/prediction 和 delta 分别使用双槽,
// 允许相邻 item 的 CrossCore 交接同时在途.
constexpr uint32_t PRED_UB_ADDR = 0;
constexpr uint32_t DELTA_UB_ADDR = PRED_UB_ADDR + CHUNK_DV_HALF_ELEMS * sizeof(float) * DB_SLOT_NUM;
constexpr uint32_t U_UB_ADDR = DELTA_UB_ADDR + STATE_HALF_ELEMS * sizeof(float) * DB_SLOT_NUM;
...
__aicore__ inline void CubeMmad(
    ...
    const AscendC::LocalTensor<BT>& bL0BLocal, uint32_t m, uint32_t n, uint32_t k, bool init = true)
{
    using namespace AscendC;
    MmadParams params;
    params.m = m;
...
__aicore__ inline void FixpipeToGmRows(
    const AscendC::GlobalTensor<DstT>& dstGlobal, const AscendC::LocalTensor<SrcT>& srcL0CLocal, uint32_t validRows,
    uint32_t cols, uint32_t srcRows, uint32_t gmRowStride)
{
    using namespace AscendC;
    FixpipeParamsArch3510<CO2Layout::ROW_MAJOR> params;
    params.mSize = validRows;
    params.nSize = cols;
    params.srcStride = CeilAlign<uint32_t>(srcRows, CUBE_BLOCK);
