...
// 每个 chunk 的只读矩阵在 L1 中整体双缓冲. MTE1 分别在 U 预取阶段读取 M,
// 在主计算阶段读取 K_plus/Q_plus/K_tail/A; 两段访问均与 MTE2 通过同一槽 Mutex 排序.
...
// Begin/End 用同一个 FlagID 交接两个 Fix 结果. Begin 取得空槽但不发布,
// End 写完第二个结果后发布; AIV 读完两个结果后归还该 FlagID.
    ...
    MutexId chunkMatrixMutexId, MutexId l0aMutexId, MutexId l0cMutexId)
{
    ...
    LoadA(uAL0Local, mL1Local, CHUNK_SIZE, CHUNK_SIZE, CHUNK_SIZE, false, l0aMutexId);
    ...
    FixToAivBegin(uSlotUBLocal, uCL0Local, CHUNK_SIZE, DV_TILE, SlotFlagId(FLAG_U_PRED_HANDOFF_BASE, slot), l0cMutexId);
}
    ...
    __gm__ bfloat16_t* valueGMAddr, __gm__ uint8_t* workspaceGMAddr, const KimiDeltaAttnLiteTilingData& data)
{
    ...
    if ASCEND_IS_AIC {
        ...
        // 每个槽打包当前 Chunk 的 KPlus/QPlus/M/KTail/A; 同一槽按 Chunk 奇偶复用.
        ...
        for (uint32_t taskId = GetBlockIdx(); taskId < data.stateNumTasks; taskId += data.stateUseAicNum) {
            ...
            if (data.chunkCount > 0) {
                ...
                auto firstKPlusL1Local =
                    firstChunkMatrixSlotL1Local[CHUNK_MATRIX_K_PLUS_SLOT_ADDR].ReinterpretCast<bfloat16_t>();
            ...
            for (uint32_t chunkId = 0; chunkId < data.chunkCount; ++chunkId) {
                ...
                if (chunkId + 1 < data.chunkCount) {
                    ...
                    auto nextKPlusL1Local =
                        nextChunkMatrixSlotL1Local[CHUNK_MATRIX_K_PLUS_SLOT_ADDR].ReinterpretCast<bfloat16_t>();
                ...
                FixToAivBegin(
                    ...
                    SlotFlagId(FLAG_HISTORY_LOCAL_HANDOFF_BASE, slot),
                    MUTEX_L0C_BASE + static_cast<MutexId>(historyCIdx));
                ...
                WaitAivToAic<PIPE_MTE1>(FLAG_R_L1_HANDOFF);
                ...
                FixToAiv(
                    deltaSlotUBLocal, deltaCL0Local, HEAD_DIM, DV_TILE, SlotFlagId(FLAG_DELTA_HANDOFF_BASE, slot),
                    MUTEX_L0C_BASE + static_cast<MutexId>(deltaCIdx));
                FixToAivEnd(
            ...
            // 等待 AIV 归还所有 AIC->AIV 交接槽, 包括奇数尾 chunk 未使用的槽.
            for (uint32_t slot = 0; slot < DB_SLOT_NUM; ++slot) {
                ...
                WaitAivToAic<PIPE_FIX>(SlotFlagId(FLAG_HISTORY_LOCAL_HANDOFF_BASE, slot));
            }
        }
    }
}
