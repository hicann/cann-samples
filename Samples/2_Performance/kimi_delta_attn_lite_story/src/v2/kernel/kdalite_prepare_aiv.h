...
__aicore__ inline void CopyPrepareNdToNzGroupsToL1(
    AscendC::LocalTensor<T>& dstL1Local, const AscendC::LocalTensor<T>& srcUBLocal)
{
    using namespace AscendC;
    constexpr uint32_t c0Elems = C0ElemNum<T>();
    DataCopyParams params(CHUNK_SIZE, 1, HEAD_DIM / c0Elems - 1, 0);
    for (uint16_t group = 0; group < HEAD_DIM / c0Elems; ++group) {
...
__aicore__ inline void IssuePrepareVpForAIV(
    ...
    const AscendC::GlobalTensor<bfloat16_t>& kTailGlobal, const AscendC::GlobalTensor<float>& stateDecayGlobal,
    const KimiDeltaAttnLiteTilingData& data, uint32_t pairTaskId, uint32_t cvSlot, uint32_t subAivIdx)
{
    ...
    if (validTask) {
        ...
        CopyPrepareNdToNzGroupsToL1(kFactorL1Local, kFactorUBLocal);
        CopyPrepareNdToNzGroupsToL1(qFactorL1Local, qFactorUBLocal);
        CopyPrepareNdToNzGroupsToL1(kInvFactorL1Local, kInvFactorUBLocal);
    ...
    const KimiDeltaAttnLiteTilingData& data, uint32_t pairTaskId, uint32_t cvSlot, uint32_t subAivIdx)
{
    ...
    if (validTask) {
        ...
        Mutex::Unlock<PIPE_V>(slotMutexId);
        // Vector 读完 Pair/Araw 后即可归还结果槽. M/A 写回 GM 由 Mutex 单独排序.
    ...
    if ASCEND_IS_AIV {
        ...
        for (uint32_t ordinal = 0; ordinal < preloadCount; ++ordinal) {
            ...
            IssuePrepareVpForAIV(
                qGlobal, kGlobal, gGlobal, betaGlobal, kPlusGlobal, qPlusGlobal, kTailGlobal, stateDecayGlobal, data,
                pairTaskId, ordinal, subAivIdx);
        }
