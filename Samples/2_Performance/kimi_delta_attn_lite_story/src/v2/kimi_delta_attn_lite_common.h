...
#ifndef KDALITE_V2_CHUNK_SIZE
#define KDALITE_V2_CHUNK_SIZE 32
#endif
constexpr uint32_t CHUNK_SIZE = KDALITE_V2_CHUNK_SIZE;
constexpr uint32_t DV_TILE = 32;
constexpr uint32_t AIV_DV_TILE = 16;

static_assert(CHUNK_SIZE == 16 || CHUNK_SIZE == 32, "v2 CHUNK_SIZE must be 16 or 32");
...
// Workspace offset 的单位均为字节. LocalTensor 的地址和元素数由 Kernel
// 根据固定 shape 计算, 不通过 TilingData 传递.
