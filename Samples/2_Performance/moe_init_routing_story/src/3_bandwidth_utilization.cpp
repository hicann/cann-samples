...
/*!
 * \file 3_bandwidth_utilization.cpp
 * \brief
 */
...
public:
                                ...
                                MoeInitRoutingTilingData *tilingData, TPipe *tPipe)
    {
        ...
        bufferNum_ = tilingData_->gatherTilingData.bufferNum;
        ...
        pipe_->InitBuffer(expandedRowIdxCopyInQueue_, bufferNum_, 
                          AlignBytes(curCorePerLoopIndicesElements_, sizeof(int32_t)));
        pipe_->InitBuffer(xCopyInQueue_, bufferNum_, AlignBytes(perLoopCols_, sizeof(float)));
        pipe_->InitBuffer(scaleCopyInQueue_, bufferNum_, AlignBytes(1, sizeof(float)));
...
void CalGatherTiling(MoeInitRoutingTilingData *tilingData)
{
    ...
    int64_t bufferNum = DOUBLE_BUFFER;
    while (perLoopMaxIndicesElements > perCoreIndicesElements && bufferNum < MAX_BUFFER_NUM) {
        bufferNum++;
        perLoopMaxIndicesElements = 
            (tilingData->ubSize / bufferNum - inputXSize - inputScaleSize) / static_cast<int64_t>(sizeof(int32_t));
    }
    gatherOutTiling->bufferNum = bufferNum;
