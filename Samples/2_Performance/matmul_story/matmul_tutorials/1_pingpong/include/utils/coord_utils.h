...
public:
        ...
        const AscendC::Std::tuple<uint32_t, uint32_t, uint32_t, uint32_t>& loadBalanceParam = {0u, 0u, 0u, 0u})
    {
        ...
        AscendC::Std::get<IDX_C_OFFSET>(offset) = mOffset * n + nOffset; // 4: idx of y
        if constexpr (isTransA) {
            AscendC::Std::get<IDX_X1SCALE_OFFSET>(offset) = mOffset * MXFP_MULTI_BASE_SIZE; // 2: idx of x1Scale
        } else {
            AscendC::Std::get<IDX_X1SCALE_OFFSET>(offset) = mOffset * CeilDiv(k, MXFP_DIVISOR_SIZE) * MXFP_MULTI_BASE_SIZE; // 2: idx of x1Scale
        }
        if constexpr (isTransB) {
            AscendC::Std::get<IDX_X2SCALE_OFFSET>(offset) = nOffset * CeilDiv(k, MXFP_DIVISOR_SIZE) * MXFP_MULTI_BASE_SIZE; // 3: idx of x2Scale
        } else {
            AscendC::Std::get<IDX_X2SCALE_OFFSET>(offset) = nOffset * MXFP_MULTI_BASE_SIZE; // 3: idx of x2Scale
        }
        return offset;
    }
