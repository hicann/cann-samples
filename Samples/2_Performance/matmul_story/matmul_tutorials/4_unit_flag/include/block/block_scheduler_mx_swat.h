 ...
 * - Group by baseM rows, column-major, alternating column direction for odd/even row groups (same as 2_aswt_block)
 * - In the last round, split the tail block into mTailTile x nTailTile pieces, shared by multiple blocks for better load balance
 */
...
public:
    ...
    int64_t blockNum_{0};
    int64_t roundIdx_{0};
    // round_: number of rounds this block executes (may differ per block due to tail-round expansion)
    int64_t round_{0};
    int64_t endBlockIdx_{0};
    int64_t mCoreNum_{0};
    int64_t mTailCoreNum_{0};
    int64_t mainRow_{0};
    // Tail block split: in the last round each tail block is split into mTailTile_ x nTailTile_ pieces, shared by multiple blocks
...
public:
    ...
    /** Extend tail-round participation: after splitting tail block into totalTailTile_ pieces, more blocks join the last round */
