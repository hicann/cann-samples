...
/*!
 * \brief MXFP4 BlockMmad: double-buffered L1/L0 + MMAD/FIXPIPE **UnitFlag**
 *        (combined with `block_scheduler_mx_last_round_tile_balance` scheduling; see `4_unit_flag`)
 */

#ifndef MATMUL_BLOCK_MMAD_MX_UNIT_FLAG_H
#define MATMUL_BLOCK_MMAD_MX_UNIT_FLAG_H
...
public:
                                   ...
                                   uint64_t scaleL1BufId, uint64_t offsetAl1, uint64_t l0cOffset)
    {
        ...
        for (uint16_t iter1 = 0; iter1 < kL0Iter; ++iter1) {
            ...
            mmadParams.unitFlag = 
                (l1Iter + 1 == kL1TileNum && iter1 + 1 == kL0Iter) ? FINAL_ACCUMULATION : NON_FINAL_ACCUMULATION;
