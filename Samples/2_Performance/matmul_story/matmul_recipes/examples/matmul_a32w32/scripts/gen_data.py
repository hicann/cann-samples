...
def write_artifacts(base_dir, a_data, b_data, out):
    ...
    a_data.view(torch.uint32).numpy().tofile(os.path.join(input_dir, "input_a.bin"))
    b_data.view(torch.uint32).numpy().tofile(os.path.join(input_dir, "input_b.bin"))
    out.view(torch.uint32).numpy().tofile(os.path.join(output_dir, "cpu_output.bin"))
...
def gen_golden_data_simple(m, k, n, transpose_a, transpose_b):
    ...
    a_ori = (np.random.uniform(0, 1, (K, M)).astype(np.float32) if transpose_a
             else np.random.uniform(0, 1, (M, K)).astype(np.float32))
    b_ori = (np.random.uniform(0, 1, (N, K)).astype(np.float32) if transpose_b
             else np.random.uniform(0, 1, (K, N)).astype(np.float32))
