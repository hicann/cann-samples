...
def _print_large_tensor_summary(golden_tensor: torch.Tensor, npu_output_tensor: torch.Tensor, m: int, n: int) -> None:
    ...
    numel = m * n
    max_ab = torch.maximum(g, p)
    ratio_rel_err = torch.where(
        max_ab > 0,
        abs_err / max_ab,
        torch.where(abs_err == 0, torch.zeros_like(abs_err), torch.full_like(abs_err, float("inf"))),
    )
    over_tol = (ratio_rel_err > RATIO_POINT_ERROR_TOL).sum().item()
    ...
    print(f"  count(rel_err(abs(a-b)/max(a,b)) > {RATIO_POINT_ERROR_TOL:g}): {over_tol} / {numel}")
...
def verify_result(m, n):
    ...
    npu_output_tensor = torch.from_numpy(output).view(torch.float32).reshape(m, n)
    golden_tensor = torch.from_numpy(golden).view(torch.float32).reshape(m, n)
    ...
    max_ab = torch.maximum(golden_f32, npu_f32)
    ratio_rel_diff = torch.where(
        max_ab > 0,
        abs_diff / max_ab,
        torch.where(abs_diff == 0, torch.zeros_like(abs_diff), torch.full_like(abs_diff, float("inf"))),
    )
    point_error_mask = (rel_diff > POINT_ERROR_TOL) | non_finite_mask
    ratio_error_mask = (ratio_rel_diff > RATIO_POINT_ERROR_TOL) | non_finite_mask
    ...
    print(
        f"ratio error count(rel_diff(abs(a-b)/max(a,b)) > {RATIO_POINT_ERROR_TOL}): {error_count}/{numel}, "
    ...
    try:
        ...
        if not res:
            raise ValueError(
                ...
                f"and the ratio of points with relative error abs(a-b)/max(a,b) > {RATIO_POINT_ERROR_TOL} "
