...
本示例展示了A32W32非量化矩阵乘算子在昇腾AI处理器上的完整实现，包含基于SWAT模板和StreamK模板的高性能优化方案。A32W32支持HF32、Float32数据类型，是深度学习中最基础且最重要的计算操作，广泛应用于各种神经网络层，包括全连接层、注意力机制等。

当前目录提供以下能力：

- `matmul_a32w32_swat`：基于SWAT模板的实现。
- `matmul_a32w32_streamk`：基于StreamK模板的实现。
- `gen_data.py`：生成输入数据和CPU golden结果。
- `verify_result.py`：校验NPU输出与CPU golden是否一致。
- `matmul_a32w32_algorithm_recommend.py`：对当前目录下可执行算法进行兼容性筛选和耗时排序。
...
- 输出矩阵C的形状为`[M, N]`。
- 支持数据类型：Float32，矩阵A,B,C数据类型需保持一致。
...
`matmul_a32w32_streamk` 对输入参数有特定的范围要求，需满足以下条件之一：
...
仓库提供 `run.sh`（位于 `matmul_recipes/examples/matmul_a32w32/scripts/`），可一键串联 **构建 → 数据生成 → 算子执行 → 结果校验** 全流程。
推荐先进入样例目录再执行，命令更短：

```bash
cd Samples/2_Performance/matmul_story/matmul_recipes/examples/matmul_a32w32
...
# 指定目标可执行文件，跳过重新构建
bash scripts/run.sh \
  --target matmul_a32w32_swat --skip-build 128 16384 128
...
如需查看完整算法推荐排名（含耗时表格），请在安装目录下直接运行 `matmul_a32w32_algorithm_recommend.py`（见下文「手动构建与运行」）。
...
cd build_out/2_Performance/matmul_story/matmul_recipes/matmul_a32w32
```
...
```bash
./matmul_a32w32_swat 128 16384 128
```

或：

```bash
./matmul_a32w32_streamk 128 16384 128
```

带转置参数：

```bash
./matmul_a32w32_swat 128 16384 128 false true
```

### 3. 运行算法推荐脚本

```bash
python3 matmul_a32w32_algorithm_recommend.py 128 16384 128
```

或带转置参数：

```bash
python3 matmul_a32w32_algorithm_recommend.py 128 16384 128 false true
```
...
+===========================+==========+=========+==========+=========+=========+============+==============+
| matmul_a32w32_swat        |    12.345|   1.234 |     0.567|   0.123 |   0.456 |     0.789 |        0.100 |
| matmul_a32w32_streamk     |    15.678|   2.100 |     0.800|   0.200 |   0.300 |     0.500 |        0.250 |
+---------------------------+----------+---------+----------+---------+---------+------------+--------------+
```
