...
# Keep verification policy aligned with quant_matmul_mxfp4/scripts/verify_result.py:
# - Single-point: relative error abs_diff/abs(golden) must be <= POINT_ERROR_TOL
# - Ratio check: fraction of points with abs error > RATIO_POINT_ERROR_TOL must be <= ERROR_RATIO_TOL
...
def verify_result(m: int, n: int) -> bool:
    """Reads ./output/npu_out.bin and ./output/cpu_output.bin (same layout for TT and TC runs)."""
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        ...
        print("  Compares ./output/npu_out.bin with ./output/cpu_output.bin")
        sys.exit(1)
