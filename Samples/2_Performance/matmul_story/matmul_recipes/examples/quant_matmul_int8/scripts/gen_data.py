...
IN_DATA_TYPE = torch.int8
OUT_DATA_TYPE = torch.int32
...
def write_artifacts(base_dir, a_data, b_data, out):
    ...
    a_data.view(torch.int8).numpy().tofile(os.path.join(input_dir, "input_a.bin"))
    b_data.view(torch.int8).numpy().tofile(os.path.join(input_dir, "input_b.bin"))
    out.view(torch.int32).numpy().tofile(os.path.join(output_dir, "cpu_output.bin"))
...
def gen_golden_data_simple(m, k, n, transpose_a, transpose_b):
    ...
    a_ori = (np.random.randint(-5, 6, (K, M)).astype(np.int8) if transpose_a
             else np.random.randint(-5, 6, (M, K)).astype(np.int8))
    b_ori = (np.random.randint(-5, 6, (N, K)).astype(np.int8) if transpose_b
             else np.random.randint(-5, 6, (K, N)).astype(np.int8))

    a_cpu = torch.from_numpy(a_ori).to(IN_DATA_TYPE)
    b_cpu = torch.from_numpy(b_ori).to(IN_DATA_TYPE)

    a_cpu_t = a_cpu.t() if transpose_a else a_cpu
    b_cpu_t = b_cpu.t() if transpose_b else b_cpu

    out = torch.matmul(a_cpu_t.to(torch.int32), b_cpu_t.to(torch.int32)).to(OUT_DATA_TYPE)
