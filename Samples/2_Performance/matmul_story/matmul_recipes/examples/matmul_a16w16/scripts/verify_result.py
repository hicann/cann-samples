...
def _print_large_tensor_summary(golden_tensor: torch.Tensor, npu_output_tensor: torch.Tensor, m: int, n: int) -> None:
    ...
    pass_ratio = (numel - over_tol) / numel * 100
...
def verify_result(m, n):
    output = np.fromfile("./output/npu_out.bin", dtype=np.uint16)
    golden = np.fromfile("./output/cpu_output.bin", dtype=np.uint16)

    if output.size != golden.size:
        raise ValueError("npu output size != cpu output size")

    npu_output_tensor = torch.from_numpy(output).reshape(m, n).to(DATA_TYPE)
    golden_tensor = torch.from_numpy(golden).reshape(m, n).to(DATA_TYPE)

    numel = m * n
    if numel <= FULL_TENSOR_PRINT_MAX_ELEMENTS:
        print("\ncpu golden:\n", golden_tensor)
        print("npu output:\n", npu_output_tensor)
        diff = torch.abs(golden_tensor - npu_output_tensor)
        over_tol = (diff > ERROR_TOL).sum().item()
        pass_ratio = (numel - over_tol) / numel
        print(f"\n[verify] shape=({m}, {n}), elements={numel}")
        print(f"  abs_err: max={diff.max().item():.6e}, mean={diff.mean().item():.6e}")
        print(f"  count(|abs_err| > {ERROR_TOL:g}): {over_tol} / {numel}")
        print(f"   pass_ratio: {pass_ratio:.6f}")
    else:
...
if __name__ == '__main__':
    if len(sys.argv) != 3:
        ...
        print('Example: python3 verify_result.py 100 200')
        sys.exit(1)
