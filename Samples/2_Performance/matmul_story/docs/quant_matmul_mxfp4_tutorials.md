...
特别说明，性能数据相差在1us以内可以认为是硬件执行带来的轻微波动，属于正常现象。
...
**Case_8192_1024_1024（多 tile、便于对比行列访问对流水的影响）**  
- Shape：`m=8192, k=1024, n=1024`。  
- 在 `baseM=baseN=256` 下，`totalCnt=32×4=128`。  
...
**Case_8448_4096_4096（多 tile、便于对比行列访问对流水的影响）**  
- Shape：`m=8448, k=512, n=4096`。  
- 在 `baseM=baseN=256` 下，`totalCnt=33*16=528`。  
- 若 `blockNum=32`，则 `round=17`：**尾轮余 16 个 tile**，每个tile**大小为（256， 256）**，如果无尾轮负载均衡，则出现16个核计算，16个核空闲的情况，而在尾轮负载均衡情况下（当前固定n方向切分成2块），则**剩余的16个tile**被切分为**32个tile**，每个**tile大小为（256, 128）**。  
