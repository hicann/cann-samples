...
![](images/ai-core-abstract-architecture.png)
...
 ![](images/ascend-a2-aic-aiv-architecture.png)
...
-   **各存储单元推荐使用的数据排布格式**
    -   L0A Buffer、L0B Buffer和L0C Buffer分别推荐采用以下分形格式（以Atlas A2 训练系列产品/Atlas A2 推理系列产品为例）：
        ...
        ![](images/l0-cube-fractal-zz-zn-nz-layout.png)
...
-   **理论分析**
    ...
    ![图4](images/cube-multicore-load-balance-tiling.png)
...
-   **理论分析**
    ...
    ![](images/split-k-multi-core-parallel-schematic.png)
...
-   **确定性切K计算**
    ...
    ![](images/split-k-deterministic-accumulation-flow.png)
...
-   **代码实践**
    ...
    -   切K前：若使用普通代码逻辑，运行耗时需要12.17us。
        ...
        ![](images/split-k-profiling-before.png)

    -   切K后：采用切K模板代码逻辑，运行耗时需要8.44us。

        ![](images/split-k-profiling-after.png)
...
-   **理论分析**
    ...
    **优化思路**：不使用随路ND2NZ指令，通过AIV核，先将数据从GM搬到UB，再通过AIV指令完成ND转NZ，然后AIC核直接读取NZ数据进L1，提升带宽效率，降低端到端耗时。
    ...
    ![](images/aiv-nd2nz-gm-ub-offload-flow.png)
...
- **代码实践**
    ...
    2.  **TransposeIn**
        ...
        ![](images/nd2nz-transpose-in-input-layout.png)

        输出排布如下：

        ![](images/nd2nz-transpose-in-output-layout.png)
    ...
    3.  **TransposeOut**
        ...
        ![](images/nd2nz-transpose-out-input-layout.png)

        输出排布如下：

        ![](images/nd2nz-transpose-out-output-layout.png)
...
-   **理论分析**
    ...
    ![](images/weight-nz-preprocess-inference-matmul.png)
...
- **理论分析**
    ...
    ![](images/dma-bank-conflict-row-col-major.png)

    为此可以使用错位分核的方式，减少M/N方向上的冲突核数，提升A/B矩阵的搬运带宽，优化整体带宽效率。

    ![](images/dma-bank-conflict-staggered-cores.png)
...
- **理论分析**
    ...
    ![](images/dual-page-table-l2-bypass-flow.png)
...
![](images/unitflag-mmad-fixpipe-block-sync.png)
...
![](images/mte2-issue-queue-full-pipeline-block.png)
...
![](images/mte2-preload-reordered-pipeline.png)
