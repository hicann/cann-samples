...
__global__ __aicore__ void MatmulKernel(GM_ADDR aGm, GM_ADDR bGm, GM_ADDR cGm, uint32_t m, uint32_t k, uint32_t n)
{
    ...
    for (uint64_t tileIdx = curBlockIdx; tileIdx < tileNum; tileIdx += blockNum) {
        ...
        // Declare that matrix tensor A needs to be fully loaded into L1
...
/**
 * @brief Determine whether full load of matrix A into L1 cache is enabled
 *
 * This function evaluates hardware constraints and kernel parameters to decide
 * if the entire A matrix can be loaded into L1 cache for better performance,
 * rather than loading tiles per chip iteration.
 *
 * @tparam T Data type of matrix elements (determines element size)
 * @param m Number of rows in matrix A
 * @param n Number of columns in matrix A (also B matrix rows)
 * @param k Number of columns in matrix B / inner dimension
 * @param numBlocks Number of thread blocks available for the kernel
 * @return true  Full load of A matrix into L1 is feasible and beneficial
 * @return false Full load not possible; fall back to tile-by-tile loading
 *
 * @note Conditions checked:
 *       1. N dimension >= 512 (sufficient size for performance benefit)
 *       2. Required tiles <= available blocks (sufficient parallelism)
 *       3. A matrix fits in half of L1 cache (256KB half of L1 minus overhead)
 *       4. Special case handling when M > 256 and M < N
 */
template <typename T>
bool IsFullLoadEnabled(int m, int n, int k, uint32_t numBlocks)
{
    // Required: Violating these conditions may cause precision issues or fail to achieve expected performance gains
    
    // Condition 1: The number of tiles must exceed the number of available cores
    int tilesM = (m + 255) / 256;
    int tilesN = (n + 255) / 256;
    bool enoughBlocks = (tilesM * tilesN >= numBlocks);

    // Condition 2: L1 capacity limit (half of L1 is 256KB, minus A matrix storage overhead)
    ...
    // Condition 3: Special scenario constraints
    // Scenario 1: M <= 256, full-load optimization takes effect (otherwise cache misses cause precision issues)
    // Scenario 2: M < N and number of tiles <= number of cores, avoids cache misses
    bool specialCondition = (m <= 256) || (m < n && (tilesM * tilesN <= numBlocks));

    // Optional: Shapes satisfying the constraints below yield better performance gains from full-load reuse
    // Condition 4: N dimension must be large enough (e.g., n >= 512) to achieve significant gains
    // Reason: Small N results in few computation rounds, limiting the benefit of full-load.
    //         Large N enables high reuse and noticeable performance improvement.
