...
constexpr static uint64_t BUFFER_GAP = 2;
...
__global__ __aicore__ void MatmulKernel(GM_ADDR aGm, GM_ADDR bGm, GM_ADDR cGm, uint32_t m, uint32_t k, uint32_t n)
{
    ...
    for (uint64_t tileIdx = curBlockIdx; tileIdx < tileNum; tileIdx += blockNum) {
        ...
        l1BufferAOffset[0] = 0;
        l1BufferAOffset[1] = AOffsetL1;
        l1BufferBOffset[0] = tool::DOUBLE_BUFFER_COUNT * AOffsetL1;
        l1BufferBOffset[1] = tool::DOUBLE_BUFFER_COUNT * AOffsetL1 + BOffsetL1;
        ...
        for (uint64_t iter0 = 0; iter0 < kL1TileNum; ++iter0) {
            ...
            auto curOffsetL1 = iter0 * kL1;
            auto curL1BufId = l1BufId;
            ...
            auto NextCurGmBKL1 = (iter0 + tool::BUFFER_GAP == kL1TileNum) ? (k - (iter0 + 1) * kL1) : kL1;
            auto NextCurGmAKL1 = NextCurGmBKL1;
            ...
            // First block: handle the first tile (tileIdx / blockNum == 0 and first iteration iter0 == 0)
            if (tileIdx / blockNum == 0 && iter0 == 0) {
                // Wait for previous MTE1 (L1 to GM) and MTE2 (GM to L1) operations to complete on this buffer
                AscendC::WaitFlag<AscendC::HardEvent::MTE1_MTE2>(l1BufId);
                
                // Create tensor for L1 buffer A and copy from global memory tile A (first slice)
                auto tensorAL1First =
                ...
                // Create tensor for L1 buffer B and copy from global memory tile B (first slice)
                auto tensorBL1First =
                ...
                // Set flag to synchronize MTE2->MTE1 after copy completes
                AscendC::SetFlag<AscendC::HardEvent::MTE2_MTE1>(l1BufId);
            }

            // Second block: prefetch next tile(s) for double buffering
            if (tileIdx / blockNum > 0 || (tileIdx / blockNum == 0 && kL1TileNum > 1 && iter0 + 1 < kL1TileNum)) {
                // For the first block's subsequent iterations, update offsets and buffer ID
                if(tileIdx / blockNum == 0) {
                    curOffsetL1 = (iter0 + 1) * kL1;
                    curGmAKL1 = NextCurGmAKL1;
                    curGmBKL1 = NextCurGmBKL1;
                    curL1BufId = 1 - l1BufId;   // toggle between two L1 buffers
                }
                
                // Wait for previous MTE1_MTE2 operations on the target buffer
                AscendC::WaitFlag<AscendC::HardEvent::MTE1_MTE2>(curL1BufId);
                
                // Recompute layouts for the next slice
                layoutAL1 = AscendC::Te::MakeLayoutAL1<T>{}(curM, curGmAKL1);
                layoutBL1 = AscendC::Te::MakeLayoutBL1<T>{}(curGmBKL1, curN);
                
                // Copy next tile of A from GM to L1 buffer (secondary buffer)
                auto tensorAL1Sec =
                ...
                // Copy next tile of B from GM to L1 buffer (secondary buffer)
                auto tensorBL1Sec =
                ...
                // Set flag after the copy is done
