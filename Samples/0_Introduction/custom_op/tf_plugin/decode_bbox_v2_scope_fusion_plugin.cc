...
#define SAMPLE_LOGE(OP_NAME, fmt, ...) printf("[ERROR]%s,%s:%u:" #fmt "\n", __FUNCTION__, __FILE__, __LINE__, ##__VA_ARGS__)
#define SAMPLE_LOGW(OP_NAME, fmt, ...) printf("[WARN]%s,%s:%u:" #fmt "\n", __FUNCTION__, __FILE__, __LINE__, ##__VA_ARGS__)
#define SAMPLE_LOGI(OP_NAME, fmt, ...) printf("[INFO]%s,%s:%u:" #fmt "\n", __FUNCTION__, __FILE__, __LINE__, ##__VA_ARGS__)
...
namespace domi {
    ...
    Status ParseFloatFromConstNode(const ge::Operator *node, float &value) {
        ...
        if (ret != ge::GRAPH_SUCCESS) {
            ...
            SAMPLE_LOGE(op_name.GetString(), "Failed to get value from %s", op_name.GetString());
            return FAILED;
        }
    ...
    Status CollectScalesInfo(const std::vector<ge::Operator> &inside_nodes,
                             std::map<std::string, std::string> &scales_const_name_map,
                             std::map<std::string, const ge::Operator *> &node_map) {
        for (const auto &node : inside_nodes) {
            ...
            if (op_type == kBoxesDiv) {
                if (node.GetInputsSize() < kRealDivInputSize) {
                    SAMPLE_LOGE(op_name.GetString(), "Input size of %s is invalid, which is %zu.", kBoxesDiv,
                            node.GetInputsSize());
                    return FAILED;
                }
    ...
    Status DecodeBboxV2ParseParams(const std::vector <ge::Operator> &inside_nodes, ge::Operator &op_dest) {
        std::map <std::string, std::string> scales_const_name_map;
        std::map<std::string, const ge::Operator *> node_map;
        auto collect_status = CollectScalesInfo(inside_nodes, scales_const_name_map, node_map);
        ...
        if (scales_const_name_map.size() != kScaleSize) {
            ...
            SAMPLE_LOGI(op_name.GetString(), "Boxes doesn't need scale.");
        } else {
            size_t i = 0;
