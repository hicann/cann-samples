...
本段落描述了 AICore 算子通过运行时接口下发任务到执行完成的完整流程。其中运行时（runtime）是华为昇腾平台CANN软件栈中负责驱动硬件执行与管理AI计算任务的核心组件，它通过提供统一的运行环境与API，使得上层应用和框架能够高效利用Ascend NPU计算资源，实现从资源初始化（例如Host和Device侧内存分配）、任务调度、模型执行的完整运行时支持。
...
| 阶段             | 说明                                  |            步骤            |
| ---------------- |-------------------------------------|:------------------------:|
| **算子调度部分** | 任务从 runtime 下发到调度到 AICore准备执行的过程    | Runtime下发（软件配置任务） + 硬件调度 |
| **算子执行部分** | AICore 执行算子的完整过程（含硬件初始化、软件初始化、算子执行） |    硬件初始化 + 软件初始化 + 计算    |

**备注**

TilingData: Tiling计算的结果，我们称之为TilingData，会传递给kernel执行时候使用。

Task Duration：算子profiling采集的算子耗时, 其中AICore调度器调度时间大约150ns，硬件初始化大约50ns，软件初始化大约1us，这个时间不是固定的由算子的TilingData大小、算子入参的个数、算子初始化行为（算子实现类成员变量初始化等）的耗时决定（例如：复杂算子的TilingData一般比较大，初始化TilingData耗时是软件头开销耗时的大头），所有核执行结束到结果上报耗时约500ns。
...
| 步骤 | 名称     | 描述                                                                                |
| :--: | -------- |-----------------------------------------------------------------------------------|
|  1   | 任务下发 | runtime将算子任务写到软件任务队列中（软件队列可以初始化在host侧内存也可以初始化在device侧内存），并发送通知给系统任务调度器            |
|  2   | 任务交换 | 系统任务调度器只能看到硬件队列中的任务，接收到步骤取得消息后，将任务从软件队列中交换到硬件队列中                                  |
|  3   | 任务解析 | 系统任务调度器接收到请求后，读取和解析硬件任务队列中的任务                                                     |
|  4   | 任务分发 | 系统任务调度器根据任务类型，将AICore任务下发到AICore调度器处理，这一步是根据任务的类型将任务分配给对应的调度器，任务调度器不同的任务都有对应的调度模块 |
|  5   | 任务调度 | AICore调度器将任务调度给AICore执行单元                                                         |

---

**备注**

任务调度器：是昇腾芯片特有的硬件调度加速模块，芯片任务和资源调度处理中心，支持调度多种计算引擎和搬运引擎，支持片上多模块的调度，将传统的软件调度硬件化，提高任务调度的效率。

AI调度器：是任务调度器中的一个模块，专门负责AIC和AIV任务的调度。

算子任务：一般一个算子的执行任务对应一个，任务内容包含了算子的类型（AIC/AIV/MIX），算子使用核数，runtime软件侧的stream id（Stream是Device提供的逻辑任务执行队列），AICore算子的执行过程中的栈地址（AICore运行时变量从寄存器溢出到内存中的地址），kernel程序在内存的位置等信息。
...
| 步骤 | 名称      | 描述                                                                                                                                             |
| :--: | --------- |------------------------------------------------------------------------------------------------------------------------------------------------|
|  1   | 系统初始化 | AICore系统控制器接收到AI调度器发过来的AICore任务，进行初始化配置（Icache Invalidate/PreLoad、Warning 清除等操作，这里的Icache Preload仅仅是将指令信息加载到内存，并不会去分析和执行），启动AICore的scalar unit |
|  2   | 指令读取 | Scalar Uint从每个AICore中的Icache中读取指令信息                                                                                                   |

**备注**

AICore系统控制器：是每个AICore内部的一个模块，负责AICore算子的初始化配置，性能统计，结果上报。

指令读取流程：当硬件读取kernel的第一条指令时，如果指令已经通过Preload到Icache，则直接返回给Scalar Uint进行解码、译码、执行等操作。如果出现了Miss，就会造成比较大的性能损失，硬件就要去Device侧内存去加载指令，由于是首次加载，指令的TLB会MISS，硬件需要去L2中查询页表，如果L2也Miss，则需要去GM获取，然后硬件根据获取到的指令的虚拟地址去L2查询，如果出现Miss，则又要去GM获取，。

Scalar unit：标量单元，除了标量计算还兼顾译码、解码、搬运和计算指令发射等功能。

### 3.2 算子软件初始化

| 步骤 | 名称       | 描述                              | 算子开发是否感知                                                  |
|:--:|----------|---------------------------------|-----------------------------------------------------------|
| 1  | 算子参数初始化  | 算子入参地址拷贝，Dcache初始化              | 否（编译器处理）                                                  |
| 2  | 全局寄存器初始化 | 全局寄存器初始化（例如控制饱和模式寄存器初始化）        | 是（例如：Ascendc初始化Tpie包含控制寄存器的初始化）                              |
| 3  | 切分参数初始化  | TilingData初始化                   | 是（例如：Ascendc获取tilingdata的宏，包含了tilingdata从device内存初始化到寄存器） |
| 4  | 其他初始化操作  | 算子内部循环边界值，切分变量和尾块大小，核地址偏移等参数初始化 | 是                                                         |

**备注**

执行顺序：步骤1是一定在算子最开始执行的，后续步骤2-4取决于算子自己调用顺序，通常按照1-4顺序处理。

算子参数：算子参数包含输入、输出、栈、TilingData指针信息，算子拷贝参数首先通过寄存器获取参数起始地址指针，然后再去GM上加载这些指信息。

TilingData初始化: 一算子初始化tilingdata就是将在device内存上的tilingData信息初始化到寄存器，如果超过寄存器上限就需要通过scalar uint的存储指令，存储到栈空间。

其他初始化操作：buffer分配（例如UB空间分配 ），初始化输入输出对于当前核的输入地址偏移，计算UB切分循环数量等操作。

Dcache： data cache的简写，区别于Icache（专门存放指令），Dcache用来存放数据，大小为32K，这里的数据一般指算子运行过程中通过Scalar的加载或者存储出去的数据（区别于通过内存搬运模块搬运的数据），Dcache也可以通过Preload指令在算子运行时手动的加载数据到Dcache。
